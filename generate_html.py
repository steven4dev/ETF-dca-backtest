  ...
  document.getElementById('conclusion').innerHTML = [
    {{
      ...
      h:`<p>標的：<span class="hl">${{ETF_DB[best].name}}</span></p>
         ...
         ${{runner(best2, best2R, best2R ? `<li>總報酬：<span class="${{best2R.returnPct>=0?'gr':'rd'}}">${{FMTP(best2R.returnPct)}}</span>　年化：${{FMTP(best2R.cagr)}}</li><li>最終市值：NT$ ${{FMT(best2R.finalValue)}}</li>` : '')}}`
    }},
    {{
      t:'🛡️ 最低回撤標的',
      h:`<p>標的：<span class="hl">${{ETF_DB[safe].name}}</span></p>
         ...
         ${{runner(safe2, safe2R, safe2R ? `<li>最大回撤：<span class="rd">${{safe2R.maxDrawdown.toFixed(2)}}%</span>　年化：${{FMTP(safe2R.cagr)}}</li>` : '')}}`
    }},
    ...(sharpR ? [{{
      t:'⚡ 最佳夏普標的',
      h:`<p>標的：<span class="hl">${{ETF_DB[sharp].name}}</span></p>
         ...
         ${{runner(sharp2, sharp2R, sharp2R ? `<li>夏普：${{sharp2R.sharpe.toFixed(2)}}　Sortino：${{fmtRisk(sharp2R.sortino)}}　年化：${{FMTP(sharp2R.cagr)}}</li>` : '')}}`
    }}] : []),
    ...(sortinoR ? [{{
      t:'🎯 最佳Sortino標的',
      h:`<p>標的：<span class="hl">${{ETF_DB[sortino].name}}</span></p>
         ...
         ${{runner(sortino2, sortino2R, sortino2R ? `<li>Sortino：${{sortino2R.sortino.toFixed(2)}}　夏普：${{fmtRisk(sortino2R.sharpe)}}　年化：${{FMTP(sortino2R.cagr)}}</li>` : '')}}`
    }}] : []),
    {{
      t:'📅 扣款日效應',
      h:`<ul>
