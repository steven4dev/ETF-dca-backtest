...
header{{
  ...
  padding:1.2rem 1.3rem .9rem;text-align:center;
}}
header h1{{font-size:1.5rem;font-weight:700;color:#fff;letter-spacing:-.5px;}}
header p{{color:var(--muted);margin-top:.15rem;font-size:.83rem;}}

/* ── Control bar ── */
.ctrl-bar{{
  background:var(--card);border-bottom:1px solid var(--border);
  padding:.65rem 1.3rem;display:flex;flex-wrap:wrap;
  align-items:center;gap:.8rem;
}}
.ctrl-group{{display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;}}
.ctrl-label{{color:var(--muted);font-size:.8rem;font-weight:600;white-space:nowrap;}}
.sep{{width:1px;height:20px;background:var(--border);}}

/* ETF 多選 */
.etf-checks{{display:flex;flex-wrap:wrap;gap:.4rem;}}
.etf-check-item{{
...
.etf-check-item input{{accent-color:var(--accent);width:13px;height:13px;cursor:pointer;}}
.etf-check-item label{{font-size:.78rem;font-weight:600;cursor:pointer;color:var(--muted);}}
.etf-check-item.checked label{{color:var(--text);}}

/* 年份按鈕 */
.year-btns{{display:flex;gap:.35rem;}}
.sel-all-btn{{
...
.amt-wrap{{
  ...
  border-radius:6px;padding:.25rem .65rem;transition:border-color .2s;
}}
.amt-wrap:focus-within{{border-color:var(--accent);}}
.amt-wrap span{{color:var(--muted);font-size:.8rem;}}
.amt-wrap input{{
...
/* ── Container ── */
.container{{max-width:1280px;margin:0 auto;padding:1.4rem 1.3rem;}}
.section-title{{
...
.kpi{{
  ...
  border-radius:9px;padding:.8rem;text-align:center;transition:border-color .2s;
}}
.kpi:hover{{border-color:var(--accent);}}
.kpi .val{{font-size:1.15rem;font-weight:700;}}
.kpi .lbl{{font-size:.68rem;color:var(--muted);margin-top:.12rem;}}
...
thead th{{
  padding:.5rem .8rem;text-align:right;color:var(--muted);
...
tbody td{{padding:.5rem .8rem;text-align:right;white-space:nowrap;}}
...
.etf-badge{{
  ...
  padding:.08rem .38rem;border-radius:4px;margin-right:.3rem;
}}
.tag{{
  display:inline-block;font-size:.6rem;padding:.07rem .32rem;
  border-radius:4px;margin-left:.22rem;font-weight:600;vertical-align:middle;
}}
...
.tab-row{{display:flex;gap:.28rem;margin-bottom:.5rem;flex-wrap:wrap;}}
.tab-btn{{
  background:var(--border);color:var(--muted);border:none;
  padding:.22rem .65rem;border-radius:5px;cursor:pointer;
  font-size:.73rem;font-weight:600;transition:all .15s;
}}
...
.c-card p,.c-card li{{font-size:.83rem;color:var(--muted);}}
.c-card ul{{padding-left:.9rem;}}
.c-card li{{margin-bottom:.22rem;}}
...
.no-data{{
  ...
  background:var(--card);border-radius:9px;border:1px dashed var(--border);
  margin-bottom:1.6rem;
}}

footer{{
  text-align:center;padding:1.1rem;color:var(--muted);
  font-size:.77rem;border-top:1px solid var(--border);
}}
</style>
</head>
<body>
