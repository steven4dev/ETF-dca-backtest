...
function renderLayoutTable() {{
  ...
  // ── 依各標的最佳報酬率計算配重 ──────────────────────────────
  // 先取各標的最佳組合報酬率，做平移使最小值 = 1%，再正規化
  const rets   = ETFS.map(id => lastResults[id][lastBestPerETF[id]].returnPct);
  const minRet = Math.min.apply(null, rets);
  const shifted = rets.map(r => Math.max(r - minRet + 1, 0.01));
  const shiftSum = shifted.reduce((s,v) => s + v, 0);
  const weights = {{}};
  ETFS.forEach((id, i) => {{ weights[id] = shifted[i] / shiftSum; }});
  ...
  for (const row of PLAN) {{
    ...
    for (const etf of ETFS) {{
      ...
      etfTotals[etf] += amt;
      const col = ETF_COLORS[etf] || '#888';
      html += '<td style="padding:.45rem .75rem;text-align:right;border-left:2px solid ' + col + '22">NT$ ' + amt.toLocaleString('zh-TW') + '</td>';
    }}
    html += '</tr>';
  }}
